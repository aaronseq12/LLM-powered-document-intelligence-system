...
class LargeLanguageModelService:
    ...
    async def enhance_extracted_data(
        ...
        try:
            ...
            chain = LLMChain(llm=self.llm, prompt=prompt_template, callbacks=[performance_tracker])
            
            result_str = await chain.arun(azure_data=json.dumps(extracted_data, indent=2))
